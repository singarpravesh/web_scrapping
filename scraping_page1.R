...
for (i in 0:3){
  ...
  # Data for price
  Price[i+1] <- remDr$findElements(using = "xpath", "//*[@class='list_header_semiBold configurationCards__configurationCardsHeading']") |> 
    sapply(function(x){x$getElementText()[[1]]})
  
  # BHK data
  Bhk[i+1] <- remDr$findElements(using = "xpath", "//*[@class='ellipsis list_header_semiBold configurationCards__configurationCardsSubHeading']") |> 
    sapply(function(x){x$getElementText()[[1]]})
 
  # Area data
  Area_sqft[i+1] <- remDr$findElements(using = "xpath", "//*[@class='caption_subdued_medium configurationCards__cardAreaSubHeadingOne']") |> 
    sapply(function(x){x$getElementText()[[1]]}) %>% 
    list()
  ...
  Latitude[i+1] <- json_ld_data[[3]]$geo$latitude
  Longitude[i+1] <- json_ld_data[[3]]$geo$longitude
 
  # Top facilities
  Top_facilities[i+1] <- read_html(html) |> 
  ...
  Other_facilities[i+1] <- read_html(html_page) |> 
